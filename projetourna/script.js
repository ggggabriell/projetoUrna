    ...
    if(candidato.length>0){
        ...
        for(let i in candidato.fotos){
            if(candidato.fotos[i].small){
                fotosHtml+= `<div class="d-1-image small"><img src="images/${candidato.fotos[i].url}" alt="candidato"/>${candidato.fotos[i].legenda}</div>`
            }else{
...
function confirma(n){
    ...
    if(votoBranco===true){
        ...
        alert("CONFIRMANDO BRANCO!");
    }else if(numero.length===etapa.numeros){
        votoConfirmado=true;
        alert("Confirmando voto em: "+numero);
        votos.push({
            etapa: etapas[etapaAtual].titulo,
            voto: numero,
        });
